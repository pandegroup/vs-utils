...
def reflect_molecule(mol_coordinates_list):
  ...
  for mol_coordinates in mol_coordinates_list:
    coordinates = deepcopy(mol_coordinates)
    reflected_coordinates = coordinates - 2. * (np.dot(v, a) / (np.dot(a, a))*a)
...
def compute_all_ecfp(system_ob, indices=None, degree=2):
  ...
  for atom in ob.OBMolAtomIter(system_ob):
    if indices is not None and atom.GetIndex() not in indices: continue
    ecfp_dict[atom.GetIndex()] = "%s,%s" %(atom.GetType(), compute_ecfp(system_ob, atom, degree))
...
def compute_ecfp_features(system_ob, ecfp_degree, ecfp_power):
  ...
  ecfp_vec = [hash_ecfp(ecfp, ecfp_power) for index, ecfp in ecfp_dict.iteritems()]
...
def featurize_binding_pocket_ecfp(protein_xyz, protein, ligand_xyz, ligand,
  ...
  protein_ecfp_dict = compute_all_ecfp(protein, indices = protein_atoms, degree = ecfp_degree)
...
def compute_splif_features_in_range(protein, ligand, pairwise_distances, contact_bin,
  ...
  protein_ecfp_dict = compute_all_ecfp(protein, indices = protein_atoms, degree = ecfp_degree)
  ligand_ecfp_dict = compute_all_ecfp(ligand, degree = ecfp_degree)
  splif_dict = {contact: (protein_ecfp_dict[contact[0]], ligand_ecfp_dict[contact[1]]) for contact in contacts}
  return(splif_dict)
...
def featurize_splif(protein_xyz, protein, ligand_xyz, ligand, contact_bins, pairwise_distances,
  ...
  for i, contact_bin in enumerate(contact_bins):
    splif_dicts.append(compute_splif_features_in_range(protein, ligand, pairwise_distances, contact_bin,ecfp_degree))
...
def is_hydrogen_bond(protein_xyz, protein, ligand_xyz, ligand, contact, hbond_angle_cutoff):
  '''
  ...
  if protein_atom.IsHbondAcceptor() and ligand_atom.IsHbondDonor():
    for atom in ob.OBAtomAtomIter(ligand_atom):
      if atom.GetAtomicNum() == 1:
        ...
        return is_angle_within_cutoff(vector_i, vector_j, hbond_angle_cutoff)
...
def compute_hbonds_in_range(protein, protein_xyz, ligand, ligand_xyz, pairwise_distances,
 hbond_dist_bin, hbond_angle_cutoff, ecfp_degree):
  ...
  contacts = np.nonzero((pairwise_distances > hbond_dist_bin[0]) & (pairwise_distances < hbond_dist_bin[1]))
  protein_atoms = set([int(c) for c in contacts[0].tolist()])
  protein_ecfp_dict = compute_all_ecfp(protein, indices=protein_atoms, degree=ecfp_degree)
  ligand_ecfp_dict = compute_all_ecfp(ligand, degree=ecfp_degree)
  contacts = zip(contacts[0], contacts[1])
  ...
  for contact in contacts:
    if is_hydrogen_bond(protein_xyz, protein, ligand_xyz, ligand, contact, hbond_angle_cutoff):
...
def convert_atom_pair_to_voxel(molecule_xyz_tuple, atom_index_pair, box_width, voxel_width):
  '''
...
class grid_featurizer:
  ...
  def transform(self, protein_pdb, ligand_pdb, save_dir):
    ...
    if "ecfp" in self.feature_types:
      ecfp_array = compute_ecfp_features(ligand_ob, self.ecfp_degree, self.ecfp_power)
    ...
    if "flat_combined" in self.feature_types:
      ...
      protein_ecfp_vector = [self.vectorize(hash_ecfp, feature_dict=protein_ecfp_dict, channel_power=self.ecfp_power)]
      ligand_ecfp_vector = [self.vectorize(hash_ecfp, feature_dict=ligand_ecfp_dict, channel_power=self.ecfp_power)]
      splif_vectors = [self.vectorize(hash_ecfp_pair, feature_dict=splif_dict, channel_power=self.splif_power) for splif_dict in splif_dicts]
      hbond_vectors = [self.vectorize(hash_ecfp_pair, feature_list=hbond_list, channel_power=0)
                             for hbond_class in hbond_list]
      feature_vectors = protein_ecfp_vector + ligand_ecfp_vector + splif_vectors + hbond_vectors
    ...
    if "voxel_combined" in self.feature_types:
      ...
      for system_id, system in transformed_systems.iteritems():
        ...
        print("Final feature tensor has size: %d" %(sys.getsizeof(feature_tensor)))
  ...
  def voxelize(self, get_voxels, hash_function, coordinates,
    ...
    feature_tensor = np.zeros((self.box_width*2,self.box_width*2,self.box_width*2, 2**channel_power),dtype=np.int8)
    if feature_dict != None:
      for key, features in feature_dict.iteritems():
        ...
        for voxel in voxels:
          try:
            feature_tensor[voxel[0],voxel[1],voxel[2],hash_function(features, channel_power)] += 1.0
          except:
            continue
  ...
  def vectorize(self, hash_function, feature_dict=None, feature_list=None, channel_power=10):
    feature_vector = np.zeros(2**channel_power)
    if feature_dict != None:
      on_channels = [hash_function(feature,channel_power) for key, feature in feature_dict.iteritems()]
